...
@section('page-title', 'Tambah Data Penilaian')
            ...
            <div class="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-navy to-navy-dark">
                <div class="flex items-center space-x-4">
                    ...
                    <div class="text-white">
                        <h3 class="text-lg font-semibold">{{ $pesertaDidik->nama_lengkap }}</h3>
                        <p class="text-blue-100 text-sm">NISN: {{ $pesertaDidik->nisn }}</p>
                    </div>
                </div>
            </div>

            <form method="POST" action="{{ route('admin.peserta-didik.penilaian.store', $pesertaDidik) }}" class="p-6">
                @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" id="tahun_ajaran" name="tahun_ajaran"
                                value="{{ old('tahun_ajaran', $pesertaDidik->tahun_ajaran) }}"
                ...
                <div class="mb-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <input type="number" id="nilai_ipa" name="nilai_ipa" value="{{ old('nilai_ipa') }}"
                        ...
                        <div>
                            ...
                            <input type="number" id="nilai_ips" name="nilai_ips" value="{{ old('nilai_ips') }}"
                                min="0" max="100" step="0.1"
                        ...
                        <div>
                            ...
                            <input type="number" id="nilai_matematika" name="nilai_matematika"
                                value="{{ old('nilai_matematika') }}" min="0" max="100" step="0.1"
                        ...
                        <div>
                            ...
                            <input type="number" id="nilai_bahasa_indonesia" name="nilai_bahasa_indonesia"
                                value="{{ old('nilai_bahasa_indonesia') }}" min="0" max="100" step="0.1"
                        ...
                        <div>
                            ...
                            <input type="number" id="nilai_bahasa_inggris" name="nilai_bahasa_inggris"
                                value="{{ old('nilai_bahasa_inggris') }}" min="0" max="100" step="0.1"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select name="minat_a" id="minat_a"
                                ...
                                <option value="Musik & Teater"
                                    {{ old('minat_a') === 'Musik & Teater' ? 'selected' : '' }}>Musik & Teater</option>
                                <option value="Fotografi & Videografi"
                                    {{ old('minat_a') === 'Fotografi & Videografi' ? 'selected' : '' }}>Fotografi &
                                    Videografi</option>
                                <option value="Seni & Kerajinan"
                                    {{ old('minat_a') === 'Seni & Kerajinan' ? 'selected' : '' }}>Seni & Kerajinan</option>
                                <option value="Desain Grafis" {{ old('minat_a') === 'Desain Grafis' ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <select name="minat_b" id="minat_b"
                                ...
                                <option value="Teknologi informasi & Komunikasi"
                                    {{ old('minat_b') === 'Teknologi informasi & Komunikasi' ? 'selected' : '' }}>Teknologi
                                    informasi & Komunikasi</option>
                                <option value="Komputer" {{ old('minat_b') === 'Komputer' ? 'selected' : '' }}>Komputer
                                </option>
                                <option value="Elektronik" {{ old('minat_b') === 'Elektronik' ? 'selected' : '' }}>
                                    Elektronik</option>
                                <option value="Mesin" {{ old('minat_b') === 'Mesin' ? 'selected' : '' }}>Mesin</option>
                            </select>
                            @error('minat_b')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <select name="minat_c" id="minat_c"
                                ...
                                <option value="Kimia" {{ old('minat_c') === 'Kimia' ? 'selected' : '' }}>Kimia</option>
                                <option value="Biologi & Lingkungan"
                                    {{ old('minat_c') === 'Biologi & Lingkungan' ? 'selected' : '' }}>Biologi & Lingkungan
                                </option>
                                <option value="Fisika" {{ old('minat_c') === 'Fisika' ? 'selected' : '' }}>Fisika</option>
                            </select>
                            @error('minat_c')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <select name="minat_d" id="minat_d"
                                ...
                                <option value="Bisnis & Enterpreneurship"
                                    {{ old('minat_d') === 'Bisnis & Enterpreneurship' ? 'selected' : '' }}>Bisnis &
                                    Enterpreneurship</option>
                                <option value="Pemasaran" {{ old('minat_d') === 'Pemasaran' ? 'selected' : '' }}>Pemasaran
                                </option>
                            </select>
                            @error('minat_d')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div>
                            ...
                            <select name="keahlian" id="keahlian"
                                ...
                                <option value="perangkat lunak"
                                    {{ old('keahlian') === 'perangkat lunak' ? 'selected' : '' }}>Perangkat Lunak</option>
                                <option value="menganalisa" {{ old('keahlian') === 'menganalisa' ? 'selected' : '' }}>
                                    Menganalisa</option>
                                <option value="kelistrikan" {{ old('keahlian') === 'kelistrikan' ? 'selected' : '' }}>
                                    Kelistrikan</option>
                                <option value="Mengembangkan Rencana & Strategi"
                                    {{ old('keahlian') === 'Mengembangkan Rencana & Strategi' ? 'selected' : '' }}>
                                    Mengembangkan Rencana & Strategi</option>
                                <option value="Menggunakan Perangkat Lunak & Komputer"
                                    {{ old('keahlian') === 'Menggunakan Perangkat Lunak & Komputer' ? 'selected' : '' }}>
                                    Menggunakan Perangkat Lunak & Komputer</option>
                                <option value="memecahkan masalah"
                                    {{ old('keahlian') === 'memecahkan masalah' ? 'selected' : '' }}>Memecahkan Masalah
                                </option>
                            </select>
                            @error('keahlian')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
